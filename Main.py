...
def enhanced_training_step(dqn, per_buffer, device):
    ...
    try:
        ...
        with torch.no_grad():
            # 找到对应的目标网络
            target_dqn = next(t_dqn for t_dqn in global_target_dqn_list if t_dqn.dqn_id == dqn.dqn_id)

            # 1. 使用当前网络 dqn (eval_net) 在下一状态 s' 中选择最佳动作 a'
            next_action_indices = dqn(next_states).argmax(dim=1, keepdim=True)

            # 2. 使用目标网络 target_dqn 评估这些动作 a' 的Q值
            target_q_from_target_net = target_dqn(next_states).gather(1, next_action_indices).squeeze(1)

            # 3. 计算最终的Q-Target
            target_q_values = rewards + RL_GAMMA * target_q_from_target_net
        # ==================== 修改后代码结束 ====================
...
def traditional_training_step(dqn, device):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            # 1. 使用当前网络 dqn (eval_net) 在下一状态 s' 中选择最佳动作 a'
            next_action_indices = dqn(next_state_tensor).argmax(dim=1, keepdim=True)

            # 2. 使用目标网络 target_dqn 评估该动作 a' 的Q值
            target_q_values = target_dqn(next_state_tensor).gather(1, next_action_indices).squeeze(1)

            # 3. 计算最终的Q-Target (确保 reward 也是 tensor)
            reward_tensor = torch.tensor([dqn.reward], dtype=torch.float32, device=device)
            dqn.q_target = (reward_tensor + RL_GAMMA * target_q_values).squeeze()
        # ==================== 修改Q-Target后代码结束 ====================
...
def rl():
    ...
    # 创建目标网络列表
    global_target_dqn_list = deepcopy(global_dqn_list)
    # 确保目标网络不参与梯度更新
    for target_dqn in global_target_dqn_list:
        for param in target_dqn.parameters():
            param.requires_grad = False

    debug_print(f"成功创建 {len(global_target_dqn_list)} 个目标DQN网络。")
    # ==================== 新增代码结束 ====================
    #from logger import debug_print
    ...
    while True:
        ...
        if epoch % SYNC_FREQUENCY == 0:
            try:
                ...
                # ==================== 新增代码开始 ====================
                # 定期更新目标网络
                SYNC_TARGET_NETWORK_FREQUENCY = 100  # 可以调整这个频率
                if epoch % SYNC_TARGET_NETWORK_FREQUENCY == 0:
                    debug_print(f"Epoch {epoch}: 同步目标网络参数...")
                    for dqn, target_dqn in zip(global_dqn_list, global_target_dqn_list):
